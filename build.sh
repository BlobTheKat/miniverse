...
OPTS="$OPTS .bin/assets.s"
export MACOSX_DEPLOYMENT_TARGET=14.6
fi

clang++ -Wno-unqualified-std-cast-call -Wno-deprecated -I.bin/include -Wfatal-errors -std=c++20 -Wno-overflow -fwrapv -Wno-narrowing$OPTS$DEFS src/main.cpp .bin/src/glad.c -L.bin/lib -lSDL2 -lSDL2main -ldl -pthread -o main && $postbuild
if [[ "$OSTYPE" == "darwin"* ]]; then
rm -f .bin/assets.s 2> /dev/null
rm -rf main.dSYM 2> /dev/null; fi